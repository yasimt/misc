...
class companyClass extends DB{
	...
	function __construct()
	{
		...
		$this->config_obj	= new configclass();				
	}
	...
	public function getData($params){		
		...
		$url_arr 	=	$this->config_obj->get_url($this->data_city);		
		$curl_url 	=	$url_arr['company_data_url']."/get";
		...
		if($curl_error_flag==1){
			$sql_err_log =	"INSERT INTO tbl_compapi_err_log SET 
			...
			$text_data['page'] 	= $page;
			$text_data['parentid'] 	= $this->parentid;
			$this->insertTextLog($text_data,1);
		}
		return $curl_res;
	}
	...
	public function updateData($params){		
		...
		if($api_invalid_flag!=1){
			$url_arr 	=	$this->config_obj->get_url($this->data_city);		
			$curl_url 	=	$url_arr['company_data_url']."/set";
		...
		//echo "<pre>";print_r($this->conn_iro);die;
