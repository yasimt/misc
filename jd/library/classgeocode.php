...
class Geocode
{
		...
		var $parentid, $lat, $lon, $latitude_primary, $ongitude_primary,$compmaster_obj;
	...
	public function __construct($dbarr, $pid, $module, $new_contract = '0', $POST = '', $IsTMENonpaid = '0')
	{
		...
		$this->compmaster_obj = new companyMasterClass($this->conn_iro,"","");
	...
	function geocode_approval_processing_and_compgeocodes($postarr)
	{
		...
		{
			//echo "locationtype--". $postarr[locationtype] ; die();
			//if($postarr[locationtype])
			switch($postarr[locationtype])
			{
				case 'building' : $geo_level = '1'; break;
				case 'landmark' : $geo_level = '2'; break;
				case 'street' 	: $geo_level = '3';	break;
				case 'pincode' 	: $geo_level = '4';	break;
				case 'area' 	: $geo_level = '6';	break;
				default :
				$geo_level = '4';
			}
			
			define('GEO_ACC_LEVEL', $geo_level); 							/* since by default pincode for new contracts */
			//$postarr[apprvd_tagging]	= "pincode";				/* By default pincode for new contracts */
			$postarr[apprvd_tagging]	= $postarr[locationtype];
	...
	function set_flagsvalue($mapPointerFlag, $flagsValue, $tagging)
	{
		...
		$mapPointerFlag = intval($mapPointerFlag);
		if( $tagging == 1 || $tagging == 2) //means building - have to set the 2nd bit
		{	
		...
		$flagsValue = intval($flagsValue);
		if( $tagging == 1 || $tagging == 2)
		{	
	...
	function select_contract_geocodes($genralInfoArr)
	{
		...
		$fieldstr	= 'latitude,longitude,geocode_accuracy_level,area,pincode';
		$tablename	= "tbl_companymaster_generalinfo";
		$whereCond	= "parentid= '".$genralInfoArr[parentid]."'";
		$this->compmaster_obj->set_datacity($genralInfoArr[parentid]);
		$temparr	=  $this->compmaster_obj->getRow($fieldstr,$tablename,$whereCond);
		$resultarr	= $temparr['data']['0'];
	...
	function UpdateOldNewAddress($IsPincodeChanged)
	{
		...
		$fieldstr	= 'building_name, landmark, street, area, pincode, city, state';
		$tablename	= "tbl_companymaster_generalinfo_shadow";
		$whereCond	= "parentid= '".$this -> parentid."'";
		$this->compmaster_obj->set_datacity($this -> parentid);
		$temparr	=  $this->compmaster_obj->getRow($fieldstr,$tablename,$whereCond);
		$row_new	= $temparr['data']['0'];
	...
	function select_generalinfo_main()
	{
		...
		$fieldstr	= 'building_name, landmark, street, area, pincode, city, state,latitude, longitude,geocode_accuracy_level';
		$tablename	= "tbl_companymaster_generalinfo_shadow";
		$whereCond	= "parentid= '".$this -> parentid."'";
		$this->compmaster_obj->set_datacity($this -> parentid);
		$temparr	=  $this->compmaster_obj->getRow($fieldstr,$tablename,$whereCond);
		$row_old	= $temparr['data']['0'];
	...
	function update_general_info_accuracy_level($accuracy_level, $table)
	{
		...
		$insertarr["tbl_companymaster_generalinfo".$tableName] = array("geocode_accuracy_level" => $accuracy_level,"parentid"=>$this -> parentid);
	...
	function fetch_live_geocode_acc_level(){
		
		$fieldstr	= 'geocode_accuracy_level';
		$tablename	= "tbl_companymaster_generalinfo";
		$whereCond	= "parentid= '".$this -> parentid."'";
		$this->compmaster_obj->set_datacity($this -> parentid);
		$temparr	=  $this->compmaster_obj->getRow($fieldstr,$tablename,$whereCond);
		
		$row_live	= $temparr['data']['0'];
		
		$live_gecode_acc_level = 'No Level Set';
		if($temparr['numrows'] > 0) {
			
			$geocde_acc_level = $row_live['geocode_accuracy_level'];
			switch($geocde_acc_level)
			{
				case 1 : 
					$live_gecode_acc_level = 'Building Level';
					break;
				case 2 :
					$live_gecode_acc_level = 'Landmark Level';
					break;
				case 3 :
					$live_gecode_acc_level = 'Street Level';
					break;
				case 4 :
					$live_gecode_acc_level = 'Pincode Level';
					break;
				case 6 :
					$live_gecode_acc_level = 'Area Level';
					break;
				default : 
					$live_gecode_acc_level = 'No Level Set';
					break;
