...
cur.execute("SELECT * FROM tbl_onboarding_invoice_details where pdf_gen=0")
#cur.execute("SELECT *FROM tbl_onboarding_invoice_details  WHERE parentid='PXX22.XX22.170710132301.X1X9'") 
...
def gethtml(row,db,city):
	...
	if city=='remote':
		rflag=1
	genio="http://genio.in/api_services/api_invoice_generation_approval_new.php?rquest=htmlpdfgen"
	
	generatedurl = genio+"&parentid="+str(row['parentid'])+"&datacity="+str(city)+"&version="+str(row['version'])+"&module=cs&rflag="+str(rflag);
