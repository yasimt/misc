...
class bformmulticity_class extends DB {
	...
	public function checkMultiCity($catidlineage_nonpaid){
		...
		$qryC 		= "select catids from tbl_business_temp_data where  contractid = '" . $this->parentid. "'";
		...
		if(strlen(trim($catstr,','))!=0)
		{
			...
			$resCat  = parent::execQuery($qryCat, $this->conn_temp);
			if($catstr)
				$rowsmfrs 	=  mysql_fetch_assoc($resCat);
				
				$nationalavail= $rowsmfrs['natCat'];
		}
	...
	public function insertNationalListingval($citystr,$latitude,$longitude,$type){
		...
		$res_insert_debug_log =  parent::execQuery($sqlIns, $this->conn_temp);
		if($resIns) {
			$res_arr['errorCode'] = "0";
