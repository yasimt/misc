...
class attribute_page_class extends DB
{
	...
	function get_vertical_flag(){
		...
		if(count($this->contract_paid_cat_arr) > 0){
			$vertical_name_arr = array();	$attribute_type_array = array();
					
			$get_attribute_type = "SELECT GROUP_CONCAT(DISTINCT(attribute_type)) as attribute_type FROM tbl_attribute_master";
			$res_attribute_type =  parent::execQuery($get_attribute_type, $this->conn_local);
			if($res_attribute_type && parent::numRows($res_attribute_type)>0){
				$row_attribute_type  = parent::fetchData($res_attribute_type);	
				
				$attribute_type = $row_attribute_type['attribute_type'];
				$attribute_type_array = explode(",", $attribute_type);
				$attribute_type_array = array_unique($attribute_type_array);
			}
			if(count($attribute_type_array)>0){
				$this->attribute_type_array = $attribute_type_array;
			}
	...
	function getAttributesInfo(){
		...
		$get_attr_names = "SELECT a.attribute_group,a.attribute_sub_group, a.unique_code, a.attribute_name,a.unique_code, a.display_position,b.attribute_type ,a.selection_flag,a.active_flag,b.input_Value,a.attribute_sub_group,a.attribute_group FROM tbl_attribute_mapping a JOIN tbl_attribute_master b USING(unique_code) WHERE a.attribute_group IN ('".$this->attribute_group."')  AND a.selection_flag=0 AND a.active_flag=1  AND a.assign_to_all=1  AND a.display_flag=1 AND  a.national_catid=0  AND  a.attribute_sub_group IN ('".$str_imp_sub."') ORDER BY FIELD (a.attribute_sub_group,'".$str_imp_sub."'),a.display_position ASC";	
		...
		//echo "<pre>data_array:--";print_r($data_array);
