...
class contract_type_class extends DB
{
	...
	{	
		...
		$data_city_arr	=	Array('mumbai','delhi','kolkata','bangalore','chennai','pune','hyderabad','ahmedabad','remote');
 		if(trim($data_city)=='' OR !in_array(trim(strtolower($data_city)),$data_city_arr)) {
			$message = "Data City is blank OR In correct.";
	...
	function get_contract_type()
	{
		...
		if(trim($this->parentid)=='' || trim($this->data_city)=='') 
		{
            $return_array['parentid'] 				=	$this->parentid;
			$return_array['contract_type'] 			=	'';
			$return_array['contract_type_display'] 	=	'';
			$return_array['paid'] 					=	'';
			$return_array['data_source']			=	$this->data_city;
			$output_final['result'] 				=   $return_array;
			$output_final['error']['code'] 			=  	'1';			
			$output_final['error']['message'] 		=  	'PARENTID/DATA CITY BLANK';			
		...
		{	
			...
			$sql_national_type="SELECT campaignid,exclusivelisting_tag,expired,balance FROM tbl_companymaster_finance_national WHERE parentid='".$this->parentid."' AND  campaignid = 10";
	...
	{
		$sql = "SELECT CorporateDealers FROM tbl_companymaster_extradetails WHERE parentid='".$this->parentid."'";
