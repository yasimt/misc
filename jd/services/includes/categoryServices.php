...
class categoryServices extends DB
{
	...
	function saveRelevantCategories(){
		if($this->mongo_flag == 1 || $this->mongo_tme == 1){
			...
			$mongo_inputs['fields'] 	= "category_flow_info";
			$data_res = $this->mongo_obj->getData($mongo_inputs);
			$temp_cat_list = $data_res['category_flow_info'];
		}else{
		...
		if($this->mongo_flag == 1 || $this->mongo_tme == 1){
			...
			$bustemp_upt = array();
			$bustemp_upt['category_flow_info'] 		= json_encode($catlist);
			$mongo_data[$bustemp_tbl]['updatedata'] = $bustemp_upt;
			
			$mongo_inputs['table_data'] 			= $mongo_data;
			$res_disc = $this->mongo_obj->updateData($mongo_inputs);			
		}
		else
		{
	...
	function getPopularAmongCompetitors(){
		...
		$existingcats=$this->getExistingCats();
		
		$temp_cat_list=json_decode($temp_cat_list,1);
		$temp_cat_list=@$temp_cat_list['MRK'];
		$temp_cat_list=str_replace("|P|", ',', $temp_cat_list);
		$temp_cat_list=explode(',', $temp_cat_list);
		if(!empty($existingcats))
		$temp_cat_list=array_merge($existingcats,$temp_cat_list);
		
		$temp_cat_list=array_filter($temp_cat_list);
		$temp_cat_list=array_unique($temp_cat_list);
		$temp_cat_list=implode(",",$temp_cat_list);
		
		$temp_cat_list=ltrim($temp_cat_list,",");
		$temp_cat_list=rtrim($temp_cat_list,",");
	...
	function getSiblingsAndChild(){
		...
		$temp_cat_list=json_decode($temp_cat_list,1);
		$temp_cat_list_m=@$temp_cat_list['MRK'];
		$temp_cat_list_p=@$temp_cat_list['POP'];
		$temp_cat_list=$temp_cat_list_m."|P|".$temp_cat_list_p;
		$temp_cat_list=$temp_cat_list_m."|P|".$temp_cat_list_p;
		$temp_cat_list=str_replace("|P|", ',', $temp_cat_list);
		$existingcats=$this->getExistingCats();
		$temp_cat_list=explode(',', $temp_cat_list);
		...
		$temp_cat_list=array_unique($temp_cat_list);
		$temp_cat_list=array_filter($temp_cat_list);
		$temp_cat_list_arr=$temp_cat_list;
		$temp_cat_list=implode(",",$temp_cat_list);
	...
	function getCatDetails($catids_search,$sameorder=0){
		...
		if($catids_search){
            ...
            $cat_params['page']   		= 'categoryServices.php';    
