...
//include_once(APP_PATH.'library/class.Curl.php');
class Virtualnumber
{
    ...
    function __construct($parentid, $dbarr, $city_vn, $usercode,$processName,$dndobj=null)
	{
		...
		$this->cityStdCode_arr =  array('MUMBAI'=>'22','AHMEDABAD'=>'79');
		$this->reliance_city = array('AHMEDABAD');
		...
		if(APP_LIVE == 1)
		{
			...
			if(VN_TECHINFO_SERVER !='')
			{
				if(in_array(strtoupper($this->city_vn),$this->main_city_arr))
				{
					...
					{
						if(in_array(strtoupper($this->city_vn),$this->reliance_city)){ 
							$this->techinfo_url="http://172.16.1.1:8100/TECDRestService/";
						}else{
	...
	function genio_update_virtual_number($parentid,$verticalFlag='',$expired_display_flag='')
    {
		...
		$extra_str="[virtual number :".intval($this->virtualno)."] [VN eligible Vertical flag : ".$verticalFlag."][JDFOS flag :".intval($this->JDFOS_flag)."]";
		$this->logmsgvirtualno("Check for Other vn eiligibale vertical",$this->log_path,$this->processName,$this->parentid,$extra_str);
		if($verticalFlag==1){
			$this->checkShopfrontFlag();
		}
        //if($this->paid_flag ==1 || $jdfocFlag ==1 || $this->JDFOS_flag)
        if($this->paid_flag ==1 || $verticalFlag ==1 || intval($this->JDFOS_flag)==1)
        { 
			$this->vnEligibleCompaign = $this->checkVnEligibleCampaign();
			//if($this->vnEligibleCompaign || $jdfocFlag ==1)
			if($this->vnEligibleCompaign || $verticalFlag ==1 || intval($this->JDFOS_flag)==1)
			{
		...
		else
		{
			if(intval($this->virtualno)>0)
			{
				...
				$extra_str="[paid flag : ".$this->paid_flag."][reason : ".$this->reason."][virtual number :".intval($this->virtualno)."][passing vertical flag:".$verticalFlag."][obj jdfos flag :".$this->JDFOS_flag."]";
	...
	function Oldexpired(){
        ...
        $qrygetAllActiveCampiagn = "SELECT count(campaignid) as linkcount FROM tbl_companymaster_finance WHERE parentid in ('".$pidstr."') AND campaignid in(1,2,23,29)";
		...
		$qry_exp_link_con_child="SELECT parentid,expired,date(expired_on) as expired_on FROM tbl_companymaster_finance WHERE parentid in ('".$pidstr."') AND campaignid in (1,2,23,29) AND expired=1 ORDER BY expired_on DESC";
		$res_exp_link_con_child = $this->execute_query($qry_exp_link_con_child,$this->conn_finance);
		if($res_exp_link_con_child){
			$totalexpiredCampaigns = mysql_num_rows($res_exp_link_con_child);
			if($totalexpiredCampaigns>0){
				$row_exp_link_con_child = mysql_fetch_assoc($res_exp_link_con_child);
				$last_expired_link_child = $row_exp_link_con_child['expired_on'];
				$last_expired_link_contract_child = $row_exp_link_con_child['parentid'];
				if($row_exp_link_con_child['parentid']!=''){
	...
	function deallocate_vn()
	{
		...
		if(in_array(strtoupper($this->city_vn),$this->main_city_arr))
		{
			...
			{
				if(in_array(strtoupper($this->city_vn),$this->reliance_city)){
					$curl_url = $this->techinfo_url."actualnumber/virtualnumber/".trim($this->virtualno)."/user/Jack";
	...
	function checkVnEligibleCampaign($pid='')
	{
		...
		$get_plat_package_budget = "SELECT balance,campaignid,expired FROM tbl_companymaster_finance WHERE parentid='".$pid."' AND campaignid in (1,2,16) order by campaignid ";
    ...
    function getEligibleLinkcontract()
    {
		...
		$where 		= "parentid IN ('".implode("','",$this->linkcontracts)."') and parentid !='".$this->parentid."' and virtualnumber>0";
	...
	function freshAllocation()
	{
		...
		if(in_array(strtoupper($this->city_vn),$this->main_city_arr))
		{
			...
			if(in_array(strtoupper($this->city_vn),$this->reliance_city)){
				...
				$curl_outout = $this->runCurlUrl($curl_url,'POST',$postarr);
			}else{
				$curl_url = $this->techinfo_url."allocate.php?";
				$urlEnd = $this->makeTechinfoUrl($businessid);
				$curl_url = $curl_url.$urlEnd;
				$curl_outout = $this->runTechInfoCurlUrl($curl_url);
			}
			//$curl_outout = '{"error":"The request has succeeded","status":200,"data":[{"virtual Number":"912230071817"}]}';
			if(!$this->curl_responce_flag)
			{
			...
			{
				if(in_array(strtoupper($this->city_vn),$this->reliance_city)){
					$tectinfo_vrn = $this->readRelianceJson($curl_outout);
				}else{
	...
	function updateJdfosDb($jdfosVno=0)
	{
		...
		$this->logmsgvirtualno("JDFOS url responce",$this->log_path,$this->processName,$this->parentid,$extra_str);
			...
			if(count($this->techInfo_array)>0)
			{
				if(trim($this->techInfo_array['BusinessId'])==trim($this->parentid))
				{
					...
					if(!$this->curl_responce_flag)
					{
						...
						$this->insert_into_log_tbl($logtbl_process_flag,$logtbl_reason,$process_name);
						$extra_str="[techinfo url not working][API OUTPUT :".$curl_url."]";
						$this->logmsgvirtualno("Allocate virtual number using techinfo",$this->log_path,$this->processName,$this->parentid,$extra_str);
					}
					else
					{
						if(in_array(strtoupper($this->city_vn),$this->reliance_city)){
							$tectinfo_vrn = $this->readRelianceJson($curl_outout,1);
						}else{
							$tectinfo_vrn = $this->readTechinfoXml($curl_outout);
						}
						$this->virtualno = $tectinfo_vrn; 
						$extra_str="[API : ".$curl_url."][POST Arr : ".json_encode($postarr)."][API OUTPUT :".$curl_outout."][vn : ".$tectinfo_vrn."][Virtual Number :".$this->virtualno."]";
					...
					if($this->linkcontract_flag)
					{
						if(in_array(trim($this->techInfo_array['BusinessId']),$this->linkcontracts))
						{
							...
							else
							{
								...
								$this->virtualno = $tectinfo_vrn;
								$extra_str="[API OUTPUT :".$curl_url."][vn : ".$tectinfo_vrn."][Virtual Number :".$this->virtualno."]";
								$this->logmsgvirtualno("Update actual number IN VN API",$this->log_path,$this->processName,$this->parentid,$extra_str);
							}
						}
						else
	...
	function makeTechinfoUrl($pid)
	{
		...
		$addition_info ="User=".trim($this->usercode)."&BusinessId=".trim($pid)."&Email=".trim($fbemail)."&Mobile=".$fb_mobile."&Contract=".trim($this->techinfoPidStatus)."&DialType=".intval($dialtype)."&BusinessName=".urlencode(trim($this->companyname));
	...
	function runCurlUrl($curl_url,$method,$postarr=array())
    {
        ...
        $this->curlobj->setOpt(CURLOPT_CONNECTTIMEOUT, 120);
        ...
        $extra_str="[Url run :".$curl_url."][method  :".$method."][post array :".json_encode($postarr)."][curl response flag : ".$this->curl_responce_flag."][curl return :".$output."]";
			...
			if(count($relianceOutput)>0){ 
				if($relianceOutput['status']=='200'){ 
					if(intval($modify_flag)==1){ 
						...
						$modifiedNUumber = $relianceOutput['data']['0']['virtual Number'];
					...
					$extra_str="[API output: ".$curl_outout."][Virtual number : ".$vn."][already assign virtual number :".$this->virtualno."]";
	...
	function vnSearch()
	{
		if(in_array(strtoupper($this->city_vn),$this->main_city_arr))
		{
			...
			if(in_array(strtoupper($this->city_vn),$this->reliance_city)){
				...
				$curl_outout = $this->runCurlUrl($curl_url,'GET');
			}else{
			...
			{
				if(in_array(strtoupper($this->city_vn),$this->reliance_city)){
					$this->techInfo_array = $this->relianceArray($curl_outout); 
				}else{
	...
	function relianceArray($curl_outout){
		$vnSearchArr = array();
		$curl_outout = trim($curl_outout);
		$vnSearchArr = json_decode($curl_outout,true);
		if(count($vnSearchArr)>0){
			if($vnSearchArr['status']==200){
				...
				$extra_str="[API OUTPUT : ".$curl_outout."][Return Reliance array :".implode($get_arry_str)."]";
				...
				$extra_str="[API OUTPUT : ".$curl_outout."][insert into logtable]";
			...
			$extra_str="[API OUTPUT : ".$curl_outout."][Reliance API Failed]";
			$this->logmsgvirtualno("Get all inforamtion from Reliance API is failed",$this->log_path,$this->processName,$this->parentid,$extra_str);
			return $this->techinfo_array;
		}
	}
	...
	function checkJdfos()
	{
		...
		else
		{
			...
			$extra_str="[jdfos curl parentid : ".$jdfos_json['results']['gen_info']['0']['parentid']."][JDFOS curl url isdeliver flag :".$jdfos_json['results']['gen_info']['0']['isDelivers']."]";
			$this->logmsgvirtualno("JDFOS url output",$this->log_path,$this->processName,$this->parentid,$extra_str);
			if(strtoupper(trim($jdfos_json['results']['gen_info']['0']['parentid'])) == strtoupper($this->parentid) && $jdfos_json['results']['gen_info']['0']['isDelivers']==1)
			{
			...
			else if(strtoupper(trim($jdfos_json['results']['gen_info']['0']['parentid'])) == strtoupper($this->parentid))
			{
				...
				$extra_str="[updated virtual number:".$this->virtualno."][ contract present on JDFOS server but is delivery flag false][jdfos_pid_chk_flag:".$this->jdfos_pid_chk_flag."]";
				$this->logmsgvirtualno("Contract Exists On JDFOS Server without delivery flag",$this->log_path,$this->processName,$this->parentid,$extra_str);
			}
			else
			{
				$this->JDFOS_flag = false;
	...
	function checkOtherVertical(){
		...
		$otherverticalFlag = 0;
		$qryCheckVerticalPayment = "SELECT parentid,campaignid FROM tbl_companymaster_finance WHERE parentid = '".$this->parentid."' AND campaignid IN (29) and balance>0";
		$resCheckVerticalPayment = $this->execute_query($qryCheckVerticalPayment,$this->conn_finance);
		if($resCheckVerticalPayment && mysql_num_rows($resCheckVerticalPayment)>0){
			$otherverticalFlag = 1;
		}else{
			$qryCheckActiveMandate = "SELECT parentid,vertical_flag FROM db_ecs.ecs_mandate WHERE parentid = '".$this->parentid."' AND deactiveflag=0 AND ecs_stop_flag=0  AND vertical_flag>0 AND mandate_type='Vertical'";
			...
			if($resCheckActiveMandate && mysql_num_rows($resCheckActiveMandate)>0){
				...
				$qryCheckActiveMandate = "SELECT parentid,vertical_flag FROM db_si.si_mandate WHERE parentid = '".$this->parentid."' AND deactiveflag=0 AND ecs_stop_flag=0 AND vertical_flag>0 AND mandate_type='Vertical'";
				$resCheckActiveMandate = $this->execute_query($qryCheckActiveMandate,$this->conn_finance);
				if($resCheckActiveMandate && mysql_num_rows($resCheckActiveMandate)>0){
					while($rowCheckActiveMandate= mysql_fetch_assoc($resCheckActiveMandate)){
						$mandateVerticalArr[] = $rowCheckActiveMandate['vertical_flag'];
	...
	function checkShopfrontFlag(){
		$this->shopfront_flag = false;
		$qryCheckShopfront = "SELECT parentid,campaignid FROM tbl_companymaster_finance WHERE parentid = '".$this->parentid."' AND campaignid = 29 AND active_campaign = 1";
		$resCheckShopfront = $this->execute_query($qryCheckShopfront,$this->conn_finance);
		if($resCheckShopfront && mysql_fetch_assoc($resCheckShopfront)>0){
			$this->shopfront_flag  = true;
		}
	}
	
	function quarantine_blockforvncat()
	{
		...
		foreach($quarantine_pid_arr as $deallocate_pid)
		{
			...
			if(intval($this->virtualno) >0)
			{
				$block_vn_quaran = chk_quaran_blockforvncat($deallocate_pid,$this->virtualno,$this->conn_decs);
				if(!$block_vn_quaran)
				{
					insert_quaran_tbl_blockforvncat($deallocate_pid,$this->virtualno,'Block For Virtual Number Category Found',$this->log_path,$this->processName,$this->usercode,$this->conn_decs);
				}
			...
			updtcompsrch_blockforvncat($deallocate_pid,$this->conn_iro);
			$this->DialableNumberClass->upDateDialableNumber($deallocate_pid,$this->conn_iro);
			updtJdfos_blockforvncat($this->jdfos_url,$deallocate_pid,$this->log_path,$this->processName,$this->usercode);
		}	
	}
	...
	function chk_block_for_vn_category()
	{
		...
		if($temparr['numrows']>0)
		{
			...
			if((isset($row_category_info['catidlineage']) && $row_category_info['catidlineage'] != '') || (isset($row_category_info['catidlineage_nonpaid']) && $row_category_info['catidlineage_nonpaid'] != ''))
			{
				...
				$sqlBlockVNCategory	=	"select count(catid) as cnt from d_jds.tbl_categorymaster_generalinfo WHERE category_type&4=4 AND catid IN ('".implode("','",$final_catlin_arr)."') GROUP BY catid";
	...
	{
		$tech_emailids = array('shakirshaikh@justdial.com','sankalpnavghare@justdial.com');
