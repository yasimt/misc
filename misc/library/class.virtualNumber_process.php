...
class Virtualnumber
{
    ...
    function __construct($parentid, $dbarr, $city_vn, $usercode,$processName,$dndobj=null)
	{
		...
		if(APP_LIVE == 1)
		{
			...
			{
				$this->mapdetails_conn    =   new DB(array(constant(strtoupper($this->city_vn).'_MAP_DETAILS_DB_IP_PROCESS'), DECS_APP_USER, DECS_APP_PASS, DB_JDS_LIVE));
		...
		else
		{
			...
			{
				$this->mapdetails_conn    =   new DB(array('192.168.6.96', 'decs_app', 's@myD#@mnl@sy', 'd_jds'));
				$this->updateMapDetails($virtualNo);
			}
		}
	}
	...
	function deallocate_vn()
	{
		...
		else if(in_array(strtoupper($this->city_vn),$this->remote_city_arr))
		{
			...
			$updtCompMaster_VN = "UPDATE tbl_companymaster_generalinfo  SET virtualNumber = '',virtual_mapped_number = '',pri_number = '' WHERE parentid = '".$this->parentid."'";
			$resCompMaster_VN = $this->execute_query($updtCompMaster_VN,$this->conn_iro);
			$tbl_mapdetails = "mapdetails_".strtolower($this->city_vn);
			if($tbl_mapdetails)
			{
				$sqlDeleteMapdetails = "DELETE FROM ".$tbl_mapdetails." WHERE LContract = '".$this->parentid."'";
	...
	{
		$tbl_mapdetails = "mapdetails_".strtolower($this->city_vn);
		$tbl_mapdetails_log = "mapdetails_log_".strtolower($this->city_vn);
		$pid_str = '';
		...
		$sql_get_matched_vn = "SELECT SVCode,Contract from ".$tbl_mapdetails." where Contract in ( '".$pid_str."')";
		...
		if($res_get_matched_vn)
		{
			...
			{
				$sqlInsertQry = "INSERT into ".$tbl_mapdetails." SET
			...
			else
			{
				...
				$sqlInsertQry = "UPDATE  ".$tbl_mapdetails." SET
								Phone      = '".$this->top_mappednumber_withoutstd[0]."',
								Email      = '".$this->email_feedback."',
								Mobile     = '".$this->mobile_feedback."',
								VrnPaidContract = '".$this->techinfoPidStatus."',
			...
			$sqlInsertLogQry = "INSERT into ".$tbl_mapdetails_log." SET
							Contract   = '".$this->parentid."',
							SVCode     = '".$virtualNo."',
							Phone      = '".$this->top_mappednumber_withoutstd[0]."',
