...
class bulk_source extends dbconn
{
	...
	function selcompanyNew($city,$cat,$source_comp,$fltr,$page,$pp,$strr,$strr1){
		...
		if( mysql_num_rows($result)!=0){
		  ...
		  $company = "select s.contactID,date(s.datesource),s.emp_detail, x.companyName,x.paid,x.freez,x.displayType,x.contract_type,x.createdby,x.mask,x.curTime  from tbl_company_source s, tbl_company_master x where s.mainsource='".$source_comp."' and x.contactID = s.contactID and s.contactID in (select y.contactid from tbl_compcatarea y where y.catidlineage like '%/".$catid[0]."/%')";
