...
function CompanyAutosuggestGeneration($parentid, $dbarr)
{
	...
	if($temparr['numrows']>0)
	{
		...
		if(count($catidArray)>0)
		{
			...
			$insert_pop_cat	= "SELECT category_name,catid FROM tbl_categorymaster_generalinfo WHERE catid IN(".$catids.") Order BY callcount DESC LIMIT 1";
		...
		if($temparr['numrows']>0)
		{
			$getdata_srch = $temparr['data']['0'];
			
			$catid_srch		= preg_replace("/[^ 0-9 ]/", ' ', $getdata_srch['catidlineage_search']);
			...
			if(count($catidSrchArr)>0)
			{
				...
				{
					$sqlFetchRestPriceRange = "SELECT DISTINCT rest_price_range FROM tbl_categorymaster_generalinfo WHERE catid IN ('".$catid_list_str."') AND category_verticals&2=2";
					$resFetchRestPriceRange = $conn_local->query_sql($sqlFetchRestPriceRange);
					if($resFetchRestPriceRange && mysql_num_rows($resFetchRestPriceRange)>0)
					{
						...
						{
							$rest_price_range_arr[] = $row_price_range['rest_price_range'];
						}
					}
				}		
			}
			...
			$insertarr['tbl_companymaster_extradetails']= array("price_range"=> $price_range);
			$wherecond = "parentid='".$parentid."'";
			$compmaster_obj->UpdateFields($insertarr,$wherecond);
		}
		/**---------Handling To Update Price Range Column In Extradetails End-----------**/
		
		if($getdata_extra['closedown_flag']==1 || $getdata_extra['closedown_flag']==13 || $getdata_extra['closedown_flag']==14 || $getdata_extra['closedown_flag']==15) //closed down company make contract and popular category filed blank
		{
			$contact=0;
			$popular_cat="";
		}		
...
function getSTDCode_WW()
{
	...
	$STDcodesql = "select stdcode from d_jds.city_master WHERE ct_name='".$cityname."' limit 1";
	$STDcoderes = $conn_iro->query_sql($STDcodesql);
...
$deletewwquery = "DELETE from tbl_autosuggest_company_whatwhere WHERE compname_area='".addslashes(stripslashes($whtwhr_array['compname_area']))."'";
