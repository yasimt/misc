...
class Virtualnumber
{
    ...
    function __construct($parentid, $dbarr, $city_vn, $usercode,$processName,$dndobj=null)
	{
        ...
        $this->DialableNumberClass = new DialableNumberClass();
	}
    ...
    {
        $vn_process_flag = true;
        $vn_process_flag = $this->chk_vn_process_eligibility();
        if($vn_process_flag == false)
        {
			$extra_str="[virtual number :".intval($this->virtualno)."]";
			$this->logmsgvirtualno("Virtual Number Process skipped : Contract Found in tbl_skip_virtualnumber_update with skip_flag 1 ",$this->log_path,$this->processName,$this->parentid,$extra_str);
			return false;
		}
        $process_flag = false;
	...
	function chk_vn_process_eligibility()
	{
		$vn_processing_flag = true;
		$sqlVNProcessEligibilty 	= "SELECT parentid FROM tbl_skip_virtualnumber_update WHERE parentid = '".$this->parentid."' AND skip_flag = '1'";
		$resVNProcessEligibilty 	= $this->execute_query($sqlVNProcessEligibilty,$this->conn_iro);
		if($resVNProcessEligibilty && mysql_num_rows($resVNProcessEligibilty)>0)
		{
			$vn_processing_flag = false;
		}
		return $vn_processing_flag;
	}
