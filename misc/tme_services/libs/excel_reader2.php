...
class Spreadsheet_Excel_Reader {
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					$response_in .= addslashes(stripslashes($val))."','";
					$out .= "</td>";
				}
			}
			$response  .= substr($response_in,0,-2);
			$out .= "</tr>\n";
			$response .= "),";
		}
		$response = substr($response,0,-1);
		$out .= "</tbody></table>";
		$response_fin['count']	=	$row;
		$response_fin['data']	=	$response;
		return $response_fin;
	}
	
	function dump_employee_data($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$data_created2	=	'';
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			$out .= "(";
			$data_created	=	'';
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					$data_created .= nl2br($val).",";
				}
			}
			$out .= substr($data_created,0,-1);
			$out .= "),";
		}
		$out .= substr($out,0,-1);
		$out .= "";
	...
	function find_parids($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$data_to_save		=	'';
		$out = "<table class=\"$table_class\" cellspacing=0>";
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
